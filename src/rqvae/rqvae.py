...
from mlp_quantizations import RQBottleneck
...
from mlp_modules import SimpleDNN
...
class RQVAE(Stage1Model):
    def __init__(self,
        ...
        self.input2 = input_dim2
        self.encoder = SimpleDNN(input_dim, [ 128], hidden_dim)
        self.decoder = SimpleDNN(hidden_dim, [128], input_dim)
        if input_dim2 is not None:
            self.encoder2 = SimpleDNN(input_dim2, [128], hidden_dim)
            self.decoder2 = SimpleDNN(hidden_dim, [128], input_dim2)
        ...
        # self.encoder2.apply(set_checkpointing)
        #
    ...
    def forward(self, xs, flag=1):
        ...
        if flag == 1:
            ...
            z_e = self.encoder2(xs)
            z_q, quant_loss, code = self.quantizer(z_e)
            out = self.decoder2(z_q)
    ...
    @torch.no_grad()
    def decode_code(self, code, flag=1):
        if self.input2 is None:
            assert flag == 1
