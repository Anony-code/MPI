...
class VQEmbedding(nn.Embedding):
    ...
    def _update_buffers(self, vectors, idxs):
        ...
        n_embed, embed_dim = self.weight.shape[0]-1, self.weight.shape[-1] # 256, 32

        vectors = vectors.reshape(-1, embed_dim) # x, 32
        idxs = idxs.reshape(-1) #

        n_vectors = vectors.shape[0] # x
        n_total_embed = n_embed # 256

        one_hot_idxs = vectors.new_zeros(n_total_embed, n_vectors)  # 256, x
...
class RQBottleneck(nn.Module):
    ...
    def __init__(self,
                 latent_shape,
                 code_shape,  # int 32 for each code book if shared
                 n_embed,  # int 256 for each codebook if shared
                 decay=0.99,
        ...
        self.code_shape = code_shape  # [32, 3]

        self.shared_codebook = shared_codebook

        self.restart_unused_codes = restart_unused_codes

        self.n_embed = [n_embed for _ in range(self.code_shape[-1])]  # [256, 256, 256]

        self.decay = decay if isinstance(decay, Iterable) else [decay for _ in range(self.code_shape[-1])]  # [0.99, 0.99, 0.99]
        assert len(self.n_embed) == self.code_shape[-1]
        assert len(self.decay) == self.code_shape[-1]

        embed_dim = code_shape[0] if isinstance(code_shape, list) else code_shape  # [32, 32, 32]

        if self.shared_codebook:
            codebook0 = VQEmbedding(self.n_embed[0],  # #card of each codebook 256
                                    embed_dim,  # the code dimension 32
                                    decay=self.decay[0],  # 0.99
                                    restart_unused_codes=restart_unused_codes,
                                    )
            self.codebooks = nn.ModuleList([codebook0 for _ in range(self.code_shape[-1])])  # shared codebooks for all
        else:
    ...
    def quantize(self, x):
        ...
        for i in range(self.code_shape[-1]):
            ...
            # find first quant, and code
            residual_feature.sub_(quant)
            aggregated_quants.add_(quant)

            quant_list.append(aggregated_quants.clone())
            code_list.append(code.unsqueeze(-1))  # first appended code is main code
    ...
    def forward(self, x):
        ...
        '''pay attention here'''
        quants_trunc = x + (quants_trunc - x).detach()  # the trunc value is not trainable
    ...
    def compute_commitment_loss(self, x, quant_list):
        ...
        """
        loss_list = []
        '''pay attention here, if needs to be changed'''
        for idx, quant in enumerate(quant_list):
            partial_loss = (x-quant.detach()).pow(2.0).mean()
            loss_list.append(partial_loss)

        commitment_loss = torch.mean(torch.stack(loss_list)) # not only the loss on the last residual, but those losses of all the residuals
    ...
    def embed_code(self, code):
        ...
        code_slices = torch.chunk(code, chunks=code.shape[-1], dim=-1)  # seperate the code to different ones

        if self.shared_codebook:
            embeds = [self.codebooks[0].embed(code_slice) for i, code_slice in enumerate(code_slices)] # using codebook to embed code to get the embeddings
        else:
            embeds = [self.codebooks[i].embed(code_slice) for i, code_slice in enumerate(code_slices)]

        embeds = torch.cat(embeds, dim=-1).sum(-1)
