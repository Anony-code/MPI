...
class Evaluator:
    def __init__(self, meta_info_path, name):
        self.name = name
        meta_info = pd.read_csv(meta_info_path, index_col=0, keep_default_na=False)
        ...
        # print(self.eval_metric, meta_info.keys(), meta_info[self.name])
    ...
    def eval2D(self, input_dict):
        ...
        hits_2d = self._2D_eval_hits(y_pred_pos, y_pred_neg, type_info)

        return hits_2d

    def eval2Dmrr(self, input_dict):
        ...
        mrr_2d = self._2D_eval_mrr(y_pred_pos, y_pred_neg, type_info)
        return mrr_2d
    ...
    def _2D_eval_hits(self, y_pred_pos, y_pred_neg, type_info):
        '''
            compute Hits@K
            For each positive target node, the negative target nodes are the same.
        ...
        if type_info == 'torch':
            kth_score_in_negative_edges = torch.topk(y_pred_neg, self.K, dim=-1)[0][:, -1]
            hitsK = float(torch.sum(y_pred_pos > kth_score_in_negative_edges).cpu()) / y_pred_pos.shape[0]
    ...
    def _eval_mrr(self, y_pred_pos, y_pred_neg, type_info):
        ...
        if type_info == 'torch':
            y_pred_pos = y_pred_pos.view(-1, 1)  # n by 1
    ...
    def _2D_eval_mrr(self, y_pred_pos, y_pred_neg, type_info):
        '''
            compute mrr
            y_pred_neg is an array with shape (batch size, num_entities_neg).
            y_pred_pos is an array with shape (batch size, )
        ...
        if type_info == 'torch':
            ...
            mrr_list = torch.mean(1./ranking_list.to(torch.float))
